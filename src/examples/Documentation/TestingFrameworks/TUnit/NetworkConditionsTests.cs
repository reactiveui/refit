...
public sealed class NetworkConditionsTests
{
    ...
    public async Task SlowServer_DelaysTheReplyWithoutWaiting()
    {
        ...
        await Assert.That(pendingReply.IsCompleted).IsFalse(); // Only one of the two simulated seconds has passed.
    ...
    public async Task BrokenNetwork_ThrowsAConnectionError()
    {
        ...
        HttpRequestException? error = await Assert.That(async () => await httpClient.GetAsync(new Uri("https://api.example.com/people/1")))
