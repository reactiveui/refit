# Testing a Refit client with MSTest

Run from `src/`: `dotnet test examples/Documentation/TestingFrameworks/MSTest/MSTestTests.csproj`.

The same scenarios exist, unchanged, for [xUnit v3](../XUnit), [NUnit](../NUnit) and [TUnit](../TUnit) — only the
test attributes and assert calls differ, so you can compare frameworks side by side.
