...
namespace Refit
{
    ...
    partial class RequestBuilderImplementation : IRequestBuilder
    {
        ...
        public RequestBuilderImplementation(Type refitInterfaceType, RefitSettings refitSettings = null)
        {
            ...
            var baseAddressAttr = refitInterfaceType.GetTypeInfo().GetCustomAttributes(true).OfType<BaseAddressAttribute>().FirstOrDefault();
            AddInterfaceHttpMethods(refitInterfaceType, baseAddressAttr, dict);
            foreach (var inheritedInterface in targetInterfaceInheritedInterfaces)
            {
                AddInterfaceHttpMethods(inheritedInterface, baseAddressAttr, dict);
            }

            interfaceHttpMethods = dict;
        }

        private void AddInterfaceHttpMethods(Type interfaceType, BaseAddressAttribute baseAddressAttr, Dictionary<string, List<RestMethodInfo>> methods)
        {
            foreach (var methodInfo in interfaceType.GetMethods())
            {
                ...
                if (hasHttpMethod)
                {
                    ...
                    var restinfo = new RestMethodInfo(interfaceType, baseAddressAttr, methodInfo, settings);
