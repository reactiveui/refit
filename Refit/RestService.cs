...
namespace Refit
{
    ...
    {
        public static T For<T>(HttpClient client, IRequestParameterFormatter requestParameterFormatter = null)
        {
            var className = "AutoGenerated" + typeof(T).Name;
            var requestBuilder = RequestBuilder.ForType<T>(requestParameterFormatter);
        ...
        public static T For<T>(string hostUrl, IRequestParameterFormatter requestParameterFormatter = null)
        {
            var client = new HttpClient() { BaseAddress = new Uri(hostUrl) };
            return RestService.For<T>(client, requestParameterFormatter);
        }
    }
}
